...
if (!empty($_POST)) {
  ...
  if (empty($_POST['producto']) || empty($_POST['precio'])) {
	...
	$lote = $_POST['lote'];
    $query_update = mysqli_query($conexion, "UPDATE producto SET fecha = '$fecha', proveedor= '$proveedor',fabricante= '$fabricante'
    ,producto='$produto', cantidad='$cantidad', fechaVencimiento='$fechaVencimiento',categoria='$categoria',numSerie='$numSerie',registroSanitario='$registroSanitario',presentacionComercial=
	'$registroSanitario', presentacionComercial='$presentacionComercial', lote='$lote' WHERE codproducto = $codproducto");
    if ($query_update) {
