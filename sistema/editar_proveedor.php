...
if (!empty($_POST)) {
  ...
  if (empty($_POST['proveedor']) || empty($_POST['contacto']) || empty($_POST['telefono']) || empty($_POST['direccion'])) {
    ...
    $sql_update = mysqli_query($conexion, "UPDATE proveedor SET proveedor = '$proveedor', contacto = '$contacto' , telefono = $telefono, direccion = '$direccion', correo = '$correo', ciudad = '$ciudad' WHERE codproveedor = $idproveedor");

    if ($sql_update) {
      $alert = '<p class="alert alert-success" role="alert">Proveedor Actualizado correctamente</p>';
    } else {
    ...
    <div class="col-lg-6 m-auto">
      ...
      <form class="" action="" method="post">
		...
		<div class="form-group">
          ...
          <input type="text" placeholder="Ingrese la Ciudad" name="ciudad" id="ciudad" class="form-control" value="<?php echo $ciudad; ?>" >
        </div>
					<!-- NUEVO POR FERNANDO 15-09 --> 
		<div class="form-group">
			<label for="correo">Correo</label>
			<input type="email" class="form-control" placeholder="Ingrese Correo Electrónico" name="correo" id="correo" value="<?php echo $correo; ?>">
		</div>
